...
<head>
    ...
    <title>Ambil Antrian - Dosen | UPA-TIK UNIMA</title>
...
<body class="min-h-screen flex items-center justify-center text-slate-100 px-4">
    <div class="glass rounded-2xl p-8 w-full max-w-md shadow-[0_0_35px_rgba(34,211,238,.1)]">
        <div class="text-center mb-8">
            ...
            <p class="text-sm text-slate-400 mt-1">Masukkan NIP Anda untuk melanjutkan</p>
        </div>
        @if($errors->any())
        <div class="bg-rose-500/10 border border-rose-400/30 text-rose-300 text-sm rounded-lg px-4 py-3 mb-5">{{ $errors->first() }}</div>
        @endif
        <form method="POST" action="{{ route('identify.dosen.submit') }}" class="space-y-4">
            @csrf
            <div>
                <label class="text-xs text-slate-400">NIP</label>
                <input type="text" name="nip" value="{{ old('nip') }}" required autofocus class="w-full mt-1 border border-cyan-400/20 rounded-lg px-4 py-2.5 text-sm">
            </div>
